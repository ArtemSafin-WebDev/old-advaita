  ...
  _autoDistance = (el, progress) => {
    ...
    if (change > 0) {
      ...
      extraChange =
        ratio === 0.5
          ? b2.height * 2
          : Math.min(b2.height, (-change * ratio) / (2 * ratio - 1)) * 2;
...
export class ScrollSmoother {
  constructor(vars) {
    ...
    let content,
      ...
      smoothDuration =
        ScrollTrigger.isTouch === 1
          ? vars.smoothTouch === true
            ? 0.8
            : parseFloat(vars.smoothTouch) || 0
          : vars.smooth === 0 || vars.smooth === false
          ? 0
          : parseFloat(vars.smooth) || 0.8,
      currentY = 0,
      delta = 0,
      startupPhase = 1,
        ...
        effects.forEach((st) => {
          ...
          triggers.forEach((trig) => {
            if (
              ...
              start = trig.start;
              dif =
                (start - st.start - st.offset) / st.ratio - (start - st.start);
        ...
        if (effects) {
          ...
          effects.forEach((st) => {
            let start = st.start,
                ...
                ? Math.min(ScrollTrigger.maxScroll(st.scroller), st.end)
      ...
      effectValueGetter = (name, value, index, el) => {
        return () => {
          ...
          v ||
            v === 0 ||
            (v = el.getAttribute("data-" + name) || (name === "speed" ? 1 : 0));
          ...
          st = ScrollTrigger.create({
            ...
            onUpdate: (self) => {
              ...
              if (self.offset) {
                ...
                markers.length &&
                  !autoSpeed &&
                  markers.forEach((setter) => setter(y - extraY));
                if (scrub) {
    ...
    this.content = function (element) {
      if (arguments.length) {
        content =
          _toArray(element || "#smooth-content")[0] || _body.children[0];
    ...
    vars.effects &&
      this.effects(
        vars.effects === true ? "[data-speed], [data-lag]" : vars.effects,
        {}
      );
      ...
      if (arguments.length) {
        if (!!paused !== value) {
          if (value) {
            ...
            mainST.progress =
              (-currentY - mainST.start) / (mainST.end - mainST.start);
            killScrub(mainST);
          }
        }
        return this;
    ...
    vars.normalizeScroll &&
      (this.observer = ScrollTrigger.normalizeScroll({ debounce: true }));
    ...
    "overscrollBehavior" in _win.getComputedStyle(_body) &&
      gsap.set(_body, { overscrollBehavior: "none" });
...
ScrollSmoother.create = (vars) =>
  _mainInstance && vars && _mainInstance.content() === _toArray(vars.content)[0]
    ? _mainInstance
